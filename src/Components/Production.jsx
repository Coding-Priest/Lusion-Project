  ...
  return (
    <div style = {{transform: 'rotateY(-180deg)'}} className="strategy-card">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 300 420" width="300" height="420">
        ...
        {/* Title */}
        <text x="20" y="35" fontFamily="Arial, sans-serif" fontSize="24" fontWeight="bold">PRODUCTION</text>
        
        {/* List items with dotted lines */}
        {['Procedural Modelling', '3D Asset Creation', '3D Asset Optimization', 'Animation', '3D Pipeline'].map((item, index) => (
        ...
        {/* Bottom text (rotated) */}
        <text x="280" y="405" fontFamily="Arial, sans-serif" fontSize="24" fontWeight="bold" transform="rotate(180 140 202.5)">PRODUCTION</text>
        
        {/* Pixelated "S" symbols */}
        {[{ x: 260, y: 20 }, { x: 20, y: 380 }].map((pos, index) => (
          <g key={index} transform={`translate(${pos.x},${pos.y}) scale(0.8)`}>
            {[
              [0,0], [5,0], [10,0], [0,5], [10,5], [0,10], [5,10], [0,15], [0,20]
            ].map(([x, y], i) => (
